...
import org.mentalizr.cicd.tasks.CleanTasks;
...
public class CleanBuildExecutor implements CommandExecutor {
    ...
    public void execute(CliCall cliCall) throws CommandExecutorException {
        ...
        TaskRunnerResult result = taskRunner.run("clean");
        if (!result.isSuccess()) throw new CommandExecutorException();
        result = taskRunner.run("init");
        if (!result.isSuccess()) throw new CommandExecutorException();
